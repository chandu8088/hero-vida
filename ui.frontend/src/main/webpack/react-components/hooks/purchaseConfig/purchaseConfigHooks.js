...
export const useGetVehicleByRegisterationNumber = () => {
  const [GetExchangeData] = useLazyQuery(GET_EXCHANGE_DATA, {
    client: apolloClient,
    fetchPolicy: "no-cache",
    onCompleted: (data) => {
      setSpinnerActionDispatcher(false);
      if (
        ...
        data.GetExchangeData.status_code !== 200
      ) {
        Logger.error(data.GetExchangeData.message);
      }
    },
    onError: (error) => {
      setSpinnerActionDispatcher(false);
