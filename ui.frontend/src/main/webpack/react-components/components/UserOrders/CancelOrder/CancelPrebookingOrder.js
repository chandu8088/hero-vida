...
const CancelPrebookingOrder = (props) => {
  ...
  const cancelPrebookingRequest = async (formData) => {
    try {
      setSpinnerActionDispatcher(true);
      const cancelUserOrderData = await cancelRequest({
        variables: {
          opportunity_id: cardData.opportunity_id,
          reason: formData.reason,
          cancellation_type: CONSTANT.CANCELLATION_TYPE.PREBOOKING
        }
      });
      myScooterDetails = await getMyScooter();
      ...
      if (
        cancelUserOrderData &&
        cancelUserOrderData.data &&
        cancelUserOrderData.data.cancellationRequest
      ) {
        if (isAnalyticsEnabled) {
          const productDetails = {
            modelVariant: cardData.productName,
        ...
        props.getAllOrders &&
          props.getAllOrders(
            cancelUserOrderData.data.cancellationRequest.status,
            cancelUserOrderData.data.cancellationRequest.message
          );
      }
    } catch (error) {
      Logger.error(error.message);
      ...
      <div className="vida-cancel-prebooking-order__container">
        <form
          ...
          onSubmit={handleSubmit((formData) =>
            cancelOrderConfig.useCancelRequest
              ? cancelPrebookingRequest(formData)
              : cancelPreBooking(formData)
          )}
        >
