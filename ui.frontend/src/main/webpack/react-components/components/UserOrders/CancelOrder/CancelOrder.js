...
const CancelOrder = (props) => {
  ...
  const cancelSaleRequest = async () => {
    console.log("===cardData", cardData);

    try {
      setSpinnerActionDispatcher(true);
      const cancellationRequest = await cancelRequest({
        variables: {
          opportunity_id: cardData.opportunity_id,
          reason: reason,
          cancellation_type: CONSTANT.CANCELLATION_TYPE.SALESORDER
        }
      });
      if (
        cancellationRequest &&
        ...
        props.getAllOrders &&
          props.getAllOrders(
            cancellationRequest.data.cancellationRequest.status,
            cancellationRequest.data.cancellationRequest.message
          );
      }
    } catch (error) {
      Logger.error(error.message);
