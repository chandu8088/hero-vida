...
const DeliveryStatusNew = (props) => {
  ...
  const [showFirstEvPopup, setShowFirstEvPopup] = useState(false);
    ...
    <>
          ...
          {isDesktop && (
            <div className="vida-billing-pricing-new__payment-container">
              <BillingPricingPaymentNew
                ...
                partialPaymentTooltipConfig={partialPaymentTooltipConfig}
              ></BillingPricingPaymentNew>
            </div>
          )}
                ...
                <>
                  ...
                  paymentMode === CONSTANT.PAYMENT_METHOD.LEASE ? (
                    ...
                    >
                      ...
                      {triggerAction === CONSTANT.PAYMENT_MODE.PARTIAL &&
                        partialPaybtn.label}
                    </button>
                  )}
                </>
      ...
      {showFirstEvPopup && (
        <div className="vida-pricing-new vida-pricing-new__popup-wrapper vida-pricing-new__subsidycheck-popup">
          <Popup
            mode="medium"
            handlePopupClose={() => setShowFirstEvPopup(false)}
          >
            <div className="vida-pricing-new__popup">
              <h4>{deliveryStatusConfig.firstTimeEv.title}</h4>
              <p>{deliveryStatusConfig.firstTimeEv.description}</p>

              <div className="vida-pricing-new__insure-btn-container">
                <button
                  className="btn btn--primary"
                  onClick={(event) => {
                    setIsFirstTimeEv(true);
                    setShowFirstEvPopup(false);
                    loanLeasePaymentHandler &&
                      loanLeasePaymentHandler(paymentMode);
                  }}
                >
                  {deliveryStatusConfig.firstTimeEv.yesBtn.label}
                </button>
                <button
                  className="btn btn--secondary"
                  onClick={(event) => {
                    setIsFirstTimeEv(false);
                    setShowFirstEvPopup(false);
                    loanLeasePaymentHandler &&
                      loanLeasePaymentHandler(paymentMode);
                  }}
                >
                  {deliveryStatusConfig.firstTimeEv.noBtn.label}
                </button>
              </div>
            </div>
          </Popup>
...
DeliveryStatusNew.propTypes = {
  ...
  setIsFirstTimeEv: PropTypes.func,
  partialPaymentTooltipConfig: PropTypes.object
};
