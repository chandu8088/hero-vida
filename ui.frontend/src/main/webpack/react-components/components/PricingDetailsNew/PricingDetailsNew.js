...
// Reverted
//import ExchangeTrackerSteps from "../PurchaseConfigurator/NewExchangeTrackerSteps/ExchangeTrackerSteps";
import {
...
const PricingDetailsNew = (props) => {
  ...
  // Reverted
  //const [showNewExchangeDetails, setShowNewExchangeDetails] = useState(false);
  ...
  const handleClosePopup = (data) => {
    ...
    // Reverted
    // setShowNewExchangeDetails(false);
    if (!tradeInSelected) {
  ...
  const handleExchageScooter = (isEdit = false) => {
    ...
    // Reverted
    // setShowNewExchangeDetails(true);
