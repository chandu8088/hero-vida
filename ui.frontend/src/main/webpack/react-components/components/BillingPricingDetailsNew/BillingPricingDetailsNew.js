...
const BillingPricingDetailsNew = (props) => {
  ...
  const [isFirstTimeEv, setIsFirstTimeEv] = useState(true);
  ...
  const partialPaymentUrl =
    appUtils.getPageUrl("partialPaymentUrl") + "?" + queryString;
  const createQueryVariables = (paymentMethod) => {
    ...
    if (paymentMethod === CONSTANT.PAYMENT_MODE.PARTIAL) {
      selectedPayment = CONSTANT.PAYMENT_METHOD.PARTIAL_PAYMENT;
    }
      ...
      <div className="vida-container">
        ...
        <div className="vida-billing-pricing-new__status">
          <DeliveryStatusNew
            ...
            hasExchangeApprovedChanged={hasExchangeApprovedChanged}
            setIsFirstTimeEv={setIsFirstTimeEv}
            partialPaymentTooltipConfig={partialPaymentTooltipConfig}
          ></DeliveryStatusNew>
          {showPopup && (
