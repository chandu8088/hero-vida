...
const BillingPricingPaymentNew = (props) => {
  ...
  const { fullPayment, loan, lease, partialPayment } = billingPricingConfig;
    ...
    <div className="vida-payment-new">
      <div className="vida-payment-new__main-wrapper">
        <div className="vida-payment-new__container">
          ...
          <div className="vida-payment-new__cash-new">
            <div className="form__group form__field-radio">
              <label className="form__field-label" htmlFor="partial">
                {partialPayment.title}
                <input
                  type="radio"
                  value={CONSTANT.PAYMENT_MODE.PARTIAL}
                  name="paymentMode"
                  id="partial"
                  onChange={changeHandler}
                />{" "}
              ...
              {partialPaymentTooltipConfig.info && (
                <>
                  <span
                    className="notification__icon"
                    data-tip={partialPaymentTooltipConfig.info}
                    data-for={partialPaymentTooltipConfig.id}
                  >
                    <i className="icon-information-circle"></i>
                  </span>
                  <ReactTooltip
                    place={partialPaymentTooltipConfig.infoPosition}
                    type="warning"
                    effect="solid"
                    id={partialPaymentTooltipConfig.id}
                  />
                </>
              )}
            </div>
...
BillingPricingPaymentNew.propTypes = {
  billingPricingConfig: PropTypes.shape({
    ...
    partialPayment: PropTypes.shape({
      title: PropTypes.string
    }),
    loan: PropTypes.object,
    lease: PropTypes.object
  }),
  partialPaymentTooltipConfig: PropTypes.shape({
